// Author: Do Khanh
// School: UIT
// Description: Layer-1 convolution processing unit for filter 3, sub-branch 1. It receives sliding-window feature data and kernel coefficients, executes INT8 multiply-accumulate cycles with controlled partial-sum updates, and emits branch-local results synchronized to pipeline valid/control timing.

`timescale 1ns / 1ps

module L1_F3_1_process_unit #(
	...
	.M0_WIDTH (M0_WIDTH)
	 ...
	 .Z_data_in('d0), .Z_weight_in('d0), .Zo_in('d57), .M0_in('d1166647221),
	 .bias_in('d8378), .n_in('d9),
